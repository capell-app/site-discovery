...
1. **Implement the four advertised health checks** — `capell.json` advertises `site-discovery.public-urls` (critical), `site-discovery.xml-sitemaps` (critical), `site-discovery.incremental` (warning), `site-discovery.html-sitemap` (warning), all pointing at `SiteDiscoveryHealthCheck`. The class is a **stub**: it implements only `compatibleCapellApiVersion(): '^4.0'` and contains no check logic — `src/Health/SiteDiscoveryHealthCheck.php`. The manifest promises critical checks that can never fail. — effort M.
2. **Schedule (or document scheduling for) `capell:xml-sitemap`** — the command is registered (`SiteDiscoveryServiceProvider::configurePackage`) but **not scheduled anywhere** in the repo. Full regeneration only happens on manual invocation; otherwise the app relies solely on event listeners firing per page/site change. Add an opt-in scheduled `--incremental` run (config-gated) so sitemaps self-heal even when events are missed (queue failures, bulk imports). — `src/Console/Commands/XmlSitemapCommand.php` — effort S.
3. **Remove the unused `icamys/php-sitemap-generator` dependency** — declared in `composer.json` `require` but **not referenced anywhere in `src/`** (XML is hand-built via `DOMDocument`/`toXml()` in `XmlSitemapGenerator`). Dead dependency adds install weight and a supply-chain surface. — `composer.json` — effort S.
...
**Screenshot/media gaps:** ship the 5 captures already specified in `docs/screenshots.json`; current single `extension-card.jpg` is insufficient. Add a parity-report screenshot showing "unknown" vs "covered" rows — it is the clearest premium-value visual.
